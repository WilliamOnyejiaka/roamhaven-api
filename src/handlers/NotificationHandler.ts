...
export default class NotificationHandler {
    ...
    public static async onConnection(io: Server, socket: ISocket) {
        ...
        const cache = await NotificationHandler.userSocketCache.get(userId);
        if (cache.error) {
        ...
        if (socketData) {
            ...
            if (await NotificationHandler.userSocketCache.set(userId, { ...socketData })) {
                console.log("successfully cached");
            }
        } else if (await NotificationHandler.userSocketCache.set(userId, { notification: socketId, chat: null })) {
